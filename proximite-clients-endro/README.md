...
1. **App personnalisée Shopify** (Admin → Paramètres → Applications et canaux de
   vente → Développer des applications → Créer une application).
   - Scope Admin API : `read_customers`.
   - Active l'accès aux **données client protégées** (Protected customer data)
     dans la config de l'app — obligatoire pour lire les adresses.
   - Installe l'app, puis copie le **jeton d'accès Admin API** (`shpat_…`).
...
3. Renseigne les **variables d'environnement** (Project → Settings → Environment
   ...
   - `SHOPIFY_STORE_DOMAIN` = `endro-cosmetiques.myshopify.com`
